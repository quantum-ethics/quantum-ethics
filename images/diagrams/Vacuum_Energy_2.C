...
main() {
	...
	text t1( fd, "\\phi_{q - q_\\gamma, \\lambda'}", f1.curve->f( 0.5 ) + f1.curve->tang( 0.5 ).perp() + xy( -1, 0 ) );
	text t2( fd, "\\overline{\\phi}_{- q, \\lambda}", f2.curve->f( 0.5 ) + f2.curve->tang( 0.5 ).perp() + xy( -1, -0.5 ) );
