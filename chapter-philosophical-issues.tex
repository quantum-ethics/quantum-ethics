...
\begin{equation*}
t_f = \mathrm{min} \{ t \in \IN\ |\ \forall t' > t,\ \cms_{t'} = \cms_t \}
\end{equation*}
...
\end{equation*}
The analysis of the asymptotical behavior can be simplified by using, for $t \geq t_f$, following factorization:
\begin{multline*}
...
\end{multline*}
The asymptotical behavior is governed by the second factor, which we will analyze under the generic form:
\begin{equation*}
...
\end{equation*}
Let $\H_\cms^n$ be the supplementary subspace of $\H_\cms^{(n + 1)+}$ in $\H_\cms^{n+}$, defined by:
\begin{equation*}
\H_\cms^{n+} = \H_\cms^{(n + 1)+} \bigoplus^\perp \H_\cms^n
\end{equation*}
