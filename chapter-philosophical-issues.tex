...
\paragraphtitle{Definitions} An infinite sequence $(\cms_t)$, indexed on $t \in \IN$, is said to be `certain' if $\mathrm P_t(\cms_0, \dotsc, \cms_t) = 1$ for all $t \in \IN$; `impossible' if $\lim_{t \to \infty} \mathrm P_t(\cms_0, \dotsc, \cms_t) = 0$; and `contingent' otherwise.
