...
Fundamentally, Quantum Field Theory also defines the probability that any given succession of collective mind states be experienced, an initial quantum state being given. Explicitely, for an initial quantum state $\ket{\Psi_i} \neq 0$ at time $t_i = 0$, the probability $\mathrm P_t(\cms_0, \dotsc, \cms_t; \ket{\Psi_i})$, where $t \in \IN$, that a given sequence $\cms_0, \dotsc, \cms_t$ is being experienced at times $0, \dotsc, t \tau$, reads for $t = 0$:
\begin{equation*}
...
\paragraphtitle{Remark} If the initial vector state isn't exactly known, but belongs to a given subspace $\H_i$, averaging on an orthonormal basis of this subspace leads to:
\begin{equation*}
