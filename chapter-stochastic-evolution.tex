...
Fundamentally, Quantum Field Theory also defines the probability that any given succession of collective mind states be experienced, an initial quantum state being given. Explicitely, for an initial quantum state $\ket{\Psi_i} \in \H_{\cms_i}$ at time $t_i = 0$, the probability $\mathrm P_t(\cms_0, \dotsc, \cms_t)$, where $t \in \IN$, that a given sequence $\cms_0, \dotsc, \cms_t$ is being experienced at times $0, \dotsc, t \tau$, reads for $t = 0$:
\begin{equation*}
\mathrm P_0(\cms_0) = \bra{\Psi_i} \op \Pi_{\cms_0} \ket{\Psi_i} / \braket{\Psi_i}{\Psi_i} = \delta_{\cms_i}(\cms_0)
\end{equation*}
for $t = 1$:
\begin{equation*}
\mathrm P_1(\cms_0, \cms_1) = \bra{\Psi_i} \op \Pi_{\cms_0} \dop U_\tau \op \Pi_{\cms_1} \op U_\tau \op \Pi_{\cms_0} \ket{\Psi_i} / \braket{\Psi_i}{\Psi_i}
\end{equation*}
...
The total probability $P_t(\cms_t)$ that a given collective mind state $\cms_t$ is being experienced at time $t \tau$, where $t \in \IN$, reads for $t = 0$:
\begin{equation*}
P_0(\cms_0) = \mathrm P_0(\cms_0)
\end{equation*}
and more generally for $t \geq 1$:
\begin{equation*}
P_t(\cms_t) = \sum_{\cms_{t - 1}} \dotsm \sum_{\cms_0} \mathrm P_t(\cms_0, \dotsc, \cms_t)
\end{equation*}

\paragraphtitle{Remark} If the initial vector state isn't exactly known, averaging on an orthonormal basis of $\H_{\cms_i}$ leads to:
\begin{equation*}
\left< P_1(\cms_1) \right>_{\cms_i} = \mathrm{Tr}_{\H_{\cms_i}} \dop U_\tau \op \Pi_{\cms_1} \op U_\tau / \mathrm{dim} \H_{\cms_i}
\end{equation*}
\begin{equation*}
\left< P_t(\cms_t) \right>_{\cms_i} = \sum_{\cms_{t - 1}} \dotsm \sum_{\cms_1} \mathrm{Tr}_{\H_{\cms_i}} \dop U_\tau \op \Pi_{\cms_1} \dotsm \dop U_\tau \op \Pi_{\cms_t} \op U_\tau \dotsm \op \Pi_{\cms_1} \op U_\tau / \mathrm{dim} \H_{\cms_i}
\end{equation*}
