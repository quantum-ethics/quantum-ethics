...
\usepackage[top=.75in,bottom=.75in,outer=.75in,inner=.75in,pdftex]{geometry}
}
...
\end{equation*}
where the summation runs over the decay states $\bFQ {N_1} \pt \q \sp$ of the radioactive atom and where the delta function can be approximated by the (time independent) density of decay states (with regard to the kinetic energy) around the ``allowed'' decay states conserving kinetic energy. The mean duration until the decay is being observed is given by the general formula:
\begin{equation*}
