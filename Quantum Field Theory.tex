...
\usepackage[paperwidth=6in,paperheight=9in,top=0.75in,bottom=0.75in,outer=0.4in,inner=0.6in,pdftex]{geometry}
}
{
\pagestyle{myheadings}
\patchcmd{\chapter}{\thispagestyle{plain}}{\thispagestyle{myheadings}}{}{}
\patchcmd{\part}{\thispagestyle{plain}}{\thispagestyle{myheadings}}{}{}
\usepackage[top=0.75in,bottom=0.75in,outer=0.75in,inner=0.75in,pdftex]{geometry}
}
...
%Cover image: Hubble telescope image of the Carina Nebula, 2010. Photograph: Nasa, ESA, M LIVIO, (STScI)/EPA
...
% Symbols
\newcommand{\RadioactiveAtom}{$\vcenter{\hbox{\includegraphics[height=.6cm]{images/pictures/radioactivity.png}}}$} % Radioactive atom
...
\newcommand{\Atom}{$\vcenter{\hbox{\includegraphics[height=.6cm]{images/pictures/atom.pdf}}}$} % Atom
\newcommand{\Radiation}{$\vcenter{\hbox{\includegraphics[height=.6cm]{images/pictures/radiation.png}}}$} % Radiation
...
{
{\LARGE Quantum Ethics} \\
--- \\
A Spinozist Interpretation \\
of Quantum Field Theory
\vskip 1cm
{\large Sébastien Fauvel\footnotemark[2]}
\vskip 5mm
{\large \today}
\footnotetext[2]{Contact: Sébastien Fauvel, Ackerstrasse~43, CH-4057 Basel, Switzerland. E-mail: \htmladdnormallink{\texttt{sebastien.fauvel@gmx.ch}}{mailto:sebastien.fauvel@gmx.ch}.}
}
\end{center}
\vfill
\null
...
These imperfections of the first edition will certainly be reason enough to publish a second, augmented edition in the future. For the time being, I wish you a fruitful and enjoyable reading and would appreciate any comments and suggestions!
...
%In the context of classical mechanics in which the Relativity Theory has been developed, and at the astronomical scale of the phenomena described by General Relativity, it has been sufficient to model observers by idealized point masses equipped with such apparatus, which aren't modeled more closely. The points of space-time in Relativity Theory should therefore be identified as macroscopic models of observers and shouldn't be identified with ; in the frame of quantum physics, however,  
...
Let us consider a simple thought experiment in order to illustrate how measurement processes take place: A radioactive atom, decaying by emitting radiations detected by a particle detector which is read by an observer. In the (ideal) initial state at time $t_0$, the radioactive atom hasn't decayed yet, the measurement apparatus is indicating that it hasn't detected any particle yet and the observer is waiting for the detector to get activated. We symbolize this situation by:
\begin{flushleft}
\RadioactiveAtom\hspace*{3mm}\DetectorOff\hspace*{3mm}\ObserverOff
\end{flushleft}
The decay of the radioactive atom into a stable atom and radiations first brings it into a quantum superposition of states, where both states coexist as a linear combination within the physical state $\ket{\psi}$ of the universe. The measurement apparatus is still inactive and the observer waiting. We symbolize this situation by:
\begin{flushleft}
\RadioactiveAtom\hspace*{3mm}\DetectorOff\hspace*{3mm}\ObserverOff
\hspace*{3mm}\Superposition\hspace*{3mm}
\Atom\hspace*{3mm}\Radiation\hspace*{3mm}\DetectorOff\hspace*{3mm}\ObserverOff
\end{flushleft}
Supposing, to simplify, that the particle detector has a detection efficiency of 100\%, the detector gets then with certainty activated by the incoming radiation, coming thus into a quantum superposition of states, where all three states coexist as a linear combination within the physical state $\ket{\psi}$ of the universe. The observer is still waiting. We symbolize this situation by:
\begin{flushleft}
\RadioactiveAtom\hspace*{3mm}\DetectorOff\hspace*{3mm}\ObserverOff
\hspace*{3mm}\Superposition\hspace*{3mm}
\Atom\hspace*{3mm}\Radiation\hspace*{3mm}\DetectorOff\hspace*{3mm}\ObserverOff
...
\end{flushleft}
Finally, at time $t_0 + \Delta t$, the observer is becoming aware of the fact that the detector has been activated and comes itself into a quantum superposition of states, so that all four states coexist as a linear combination within the physical state $\ket{\psi}$ of the universe. We symbolize this situation by:
\begin{flushleft}
\RadioactiveAtom\hspace*{3mm}\DetectorOff\hspace*{3mm}\ObserverOff
\hspace*{3mm}\Superposition\hspace*{3mm}
\Atom\hspace*{3mm}\Radiation\hspace*{3mm}\DetectorOff\hspace*{3mm}\ObserverOff
...
\end{flushleft}
This (purely physical) situation lasts until the next process of collapse and collective mind selection takes place. The experienced consciousness state of the observer is then either one of ``I am still waiting for the detector to get activated'' or ``I have seen the detector becoming activated''. In the first case, the physical state of the universe becomes again:
\begin{flushleft}
\RadioactiveAtom\hspace*{3mm}\DetectorOff\hspace*{3mm}\ObserverOff
\hspace*{3mm}\Superposition\hspace*{3mm}
\Atom\hspace*{3mm}\Radiation\hspace*{3mm}\DetectorOff\hspace*{3mm}\ObserverOff
...
\end{equation*}
and the mean duration until the atomic decay is being observed can be approximated by:
\begin{equation*}
\left< t - t_0 \right> \approx \Delta t + \h \Big[ ( 2 \pi )^2 \sum_{\bFQ {N_1} \pt \q \sp} \left| H'_{1,i} \right|^2 \deltaE2{E_1 - E_i} \Big]^{-1}
\end{equation*}
where the summation runs over the decay states $\bFQ {N_1} \pt \q \sp$ of the radioactive atom. The delta function can be approximated by the (time independent) density of decay states (with regard to the kinetic energy) around the ``allowed'' decay states conserving kinetic energy.
...
%TODO: Remarques sur géométrie torique (expliciter et faire le lien avec la remarque 1.2) et quantisation de la vitesse. q_0 réel possible? bremsstrahlung + mass renormalization

\chapter{Coulomb scattering}
\label{Coulomb scattering}

%\section{Abstract}
%The aim of this chapter is to derive the differential cross sections of some classical scattering experiments by using simple, semi-classical expressions for the interaction Hamiltonian.
...
\bibitem{Descartes1637} Descartes, R. (1637). \textit{Discours de la méthode pour bien conduire sa raison, et chercher la vérité dans les sciences}. Ian Maire, Leiden (Netherlands), 1637.

\bibitem{Einstein1916} Einstein, A. (1916). \textit{Über die spezielle und die allgemeine Relativitätstheorie}. Vieweg, Braunschweig, 1917. \htmladdnormallink{\textit{Relativity: The Special and General Theory}}{http://www.gutenberg.org/ebooks/5001}. Methuen \& Co. Ltd., London (United Kingdom), 1920.
...
\bibitem{Freedman2001} Freedman, W. L.; et al. (2001). \htmladdnormallink{\textit{Final Results from the Hubble Space Telescope Key Project to Measure the Hubble Constant}}{http://arxiv.org/abs/astro-ph/0012376v1}. Astrophysical Journal 553 (1): 47–72.
...
\bibitem{Kogut1993} Kogut, A.; et al. (1993). \htmladdnormallink{\textit{Dipole Anisotropy in the COBE Differential Microwave Radiometers First-Year Sky Maps}}{http://arxiv.org/abs/astro-ph/9312056v1}. Astrophysical Journal 419: 1–6.

\bibitem{Komatsu2010} Komatsu, E.; et al. (2010). \htmladdnormallink{\textit{Seven-Year Wilkinson Microwave Anisotropy Probe (WMAP) Observations: Cosmological Interpretation}}{http://arxiv.org/abs/1001.4538v2}. Astrophysical Journal Supplement Series 192: 18, 2011.

\bibitem{Marin2009} Marin, J. M. (2009). \htmladdnormallink{\textit{‘Mysticism’ in quantum mechanics: the forgotten controversy}}{http://harvard.academia.edu/JuanMiguelMarin/Papers/204341}. European Journal of Physics 30: 807-822, 2009.

\bibitem{Neumann1932} Neumann, J. v. (1932). \textit{Mathematische Grundlagen der Quantenmechanik}. Julius Springer, Berlin, 1932. \textit{Mathematical Foundations of Quantum Mechanics}. Princeton University Press, Princeton (New Jersey, United States), 1955.

\bibitem{Rabin1981} Rabin, J. M. (1981). \htmladdnormallink{\textit{Long Range Interactions In Lattice Field Theory}}{http://www.slac.stanford.edu/pubs/slacreports/slac-r-240.html}. Ph.D. Thesis. SLAC-0240.

\bibitem{Spinoza1677} Spinoza, B. (1677). \htmladdnormallink{\textit{The Ethics (Ethica Ordine Geometrico Demonstrata)}}{http://www.gutenberg.org/ebooks/3800}.
