...
\usepackage[paperwidth=6in,paperheight=9in,top=0.75in,bottom=0.75in,outer=0.4in,inner=0.75in,pdftex]{geometry}
}
{
\pagestyle{myheadings}
\patchcmd{\chapter}{\thispagestyle{plain}}{\thispagestyle{myheadings}}{}{}
...
\vskip 5mm
{\large \today}
\footnotetext[2]{Contact: Sébastien Fauvel, Ackerstrasse~43, CH-4057 Basle, Switzerland. E-mail: \htmladdnormallink{\texttt{sebastien.fauvel@gmx.ch}}{mailto:sebastien.fauvel@gmx.ch}.}
}
\end{center}
\vfill
\null
...
\paragraphtitle{Definition} The (hypothetical) physical state in which the universe only contains a single particle, of type $\pt$, at point $\n$ and in the spin state $\sp$, is written:
\begin{equation*}
\ket{\Psi} = \ketX 1 \pt \n \sp
\end{equation*}
...
\section{Fermion antisymmetrization operator}
\label{Antisymmetrization operator}

Let us define first a standard order on $\FLD$, also a total order relation given by $\q < \q'$ if and only if one of the following assertions holds:
\begin{equation*}
\begin{aligned}[c]
q_1 < q_1'
\end{aligned}
...
\end{cases}
\end{equation*}
The fermion antisymmetrization operators $\Faop \pt \q \sp$ can be defined with the help of this standard particle numbering function by their action on the momentum basis:
\begin{eqnarray*}
...
\end{eqnarray*}
These hermitian, unitary operators are always used together with the corresponding creation and annihilation operators for fermion fields. They verify following anticommutation properties, where the anticommutation notation $\{\cdot, \cdot\}$ is defined by $\{ \op a, \op b\} \eqdef \op a \op b + \op b \op a$:
\begin{equation*}
...
\end{eqnarray*}
where the fermion antisymmetrization operators $\Faop \pt \q \sp$ are defined as in appendix \ref{Antisymmetrization operator}. They can also be defined on the position basis by:
\begin{eqnarray*}
...
% Sébastien Fauvel, born 1983, is an alumnus of the Ecole Normale Supérieure of Paris, where he graduated in Physics and Comparative literature. He has been working as a Consultant, Software and Web Developer in Lyon, Freiburg and Basle.
